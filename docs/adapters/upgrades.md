...
For a direct `0.2.1` check, `0.2.0` is the supported upgrade source and `0.2.1` is the target.
...
and the final revision must target the running `0.2.1` core. Repeated versions are stale;
